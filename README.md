...
- **An entrant that can't be identified at all still gets a visible flag.** If an enter (or "looks around.") line is anonymous and has no matching orphan to borrow an identity from, the line is still hidden, but a distinctly-styled "⚠ Unregistered person — do /look" row appears in the Present tab instead of the activity just vanishing without a trace. It's replaced automatically the moment a later line resolves who they were, or cleared by the next `/look` resync either way.
- **New arrivals flash and stay highlighted.** Whenever someone actually enters (not your own seed entry, someone waking from torpor, or a routine `/look` resync), the Present toggle button pulses orange so it's noticeable even with the tab closed, and their row keeps a fading orange background for a while afterward so a fresh face is easy to spot at a glance.
...
- `NEW_ARRIVAL_HIGHLIGHT_MS` — how long a freshly-added roster row keeps its highlighted background before fading back to normal (default `15000`, i.e. 15 seconds).
