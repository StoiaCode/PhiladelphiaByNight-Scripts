...
- **Movement grouping** — every `[SYSTEM]` narrative line (walk in, look around, walk out, and anything else we can't specifically classify) renders in a muted grey tone as soon as it arrives, so none of it stands out the way an important system message would — even lines about characters we can't identify at all. When the same person's (or a "follows" chain's) lines arrive back to back, they morph into one compact block: the name shown once, each original line underneath as its own short row, in that same tone. This still works even when a follower has no proper name — some characters show an anonymous/masked description instead (e.g. "an immaculately dressed, but horribly unkempt lady") — as long as their line names who they're following, it bridges into that person's group. A genuinely unrelated person's line still starts a fresh block. (Two *anonymous* lines about the same masked character that aren't connected by a "follows" mention can't currently be linked to each other — they're each muted individually rather than merged, since there's no reliable way to tell they're the same person without a name.)
